...
def run():
    ...
    try:
        ...
        if now < entry or now > cutoff:
            print("[CALL] Outside trade window")
            return
        ...
        short_call = Option("SPX", expiry, short_strike, "C", "CBOE", tradingClass="SPXW")
        long_call = Option("SPX", expiry, long_strike, "C", "CBOE", tradingClass="SPXW")
        ib.qualifyContracts(short_call, long_call)
        ...
        bag = Bag(
            ...
            currency="USD",
            comboLegs=[
                ComboLeg(conId=short_call.conId, ratio=1, action="SELL", exchange="CBOE"),
                ComboLeg(conId=long_call.conId, ratio=1, action="BUY", exchange="CBOE"),
            ],
        )

        mid = spread_mid_credit(ib, short_call, long_call)
        credit = MIN_CREDIT if is_bad_px(mid) else max(MIN_CREDIT, round(mid, 2))

        print(f"[CALL] mid={mid} → credit={credit}")
        ...
        while True:
            ...
            print(f"[CALL] HOLD credit={credit} status={st}")
            if st == "Filled":
                print("[CALL] FILLED")
                return
            time.sleep(120)
