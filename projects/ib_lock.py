...
def connect_checked() -> tuple[IB, IBConnCfg, str]:
    cfg = load_cfg()
    ib = IB()
...
if __name__ == "__main__":
    # run both checks in one go (no manual export needed)
    for env_path in ("/root/projects/.env_live", "/root/projects/.env_paper"):
        clear_env_keys()
        load_env(env_path)
        ib = None
        try:
            ib, cfg, acct = connect_checked()
            print(f"OK ✅ env={env_path} mode={cfg.mode} port={cfg.port} clientId={cfg.client_id} acct={acct}")
        finally:
            try:
                if ib:
                    ib.disconnect()
